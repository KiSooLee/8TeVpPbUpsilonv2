...
void Correl_peak_bkg(bool isMC = false, const Int_t MultMin = 0, const Int_t MultMax = 300, const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t TrkptMin = 0, Double_t TrkptMax = 1, TString version = "v1")
{
	TString DM;
	if(isMC) DM = "MC";
	else DM = "RD";
...
//Get files{{{
	...
	fname2 = Form("SkimmedFiles/Sort_OniaTree_PADoubleMuon_%s_Mult_%d-%d_pt_%d-%d_Trkpt_%d-%d_%s_1.root", DM.Data(), MultMin, MultMax, (int)ptMin, (int)ptMax, (int)TrkptMin, (int)TrkptMax, version.Data());
...
//}}}
	...
	c1->cd();
	h1->Draw("LEGO");
	c1->SaveAs(Form("deta-dphi_%s_distribution_peak_bkg_Mult_%d-%d_pt_%d-%d_Trkpt_%d-%d_%s.pdf", DM.Data(), MultMin, MultMax, (int)ptMin, (int)ptMax, (int)TrkptMin, (int)TrkptMax, version.Data()));

	TFile* fout = new TFile(Form("deta-dphi_%s_distribution_peak_bkg_Mult_%d-%d_pt_%d-%d_Trkpt_%d-%d_%s.root", DM.Data(), MultMin, MultMax, (int)ptMin, (int)ptMax, (int)TrkptMin, (int)TrkptMax, version.Data()), "RECREATE");
	fout->cd();
	h1->Write();
	fout->Close();
}
