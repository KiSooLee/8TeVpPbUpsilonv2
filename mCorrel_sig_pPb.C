...
void mCorrel_sig_pPb(bool isMC = false, const Int_t multMin = 0, const Int_t multMax = 300, const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, TString version = "v1", const Int_t imass = 0, const Int_t nmbin = 20)
{
/*
//Make directory{{{
...
//Get files{{{
	...
	{
		fname1 = Form("root://cms-xrdr.sdfarm.kr:1094///xrd/store/user/kilee/pPb_8TeV_OniaTrkTree/resultpPb1/%d-%d_%d-%d_%d-%d_%d-%d/Sort_OniaTree_pPb1_PADoubleMuon_%d.root", (int)multMin, (int)multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, imass*(120/nmbin)+ibin);
		fname2 = Form("root://cms-xrdr.sdfarm.kr:1094///xrd/store/user/kilee/pPb_8TeV_OniaTrkTree/resultpPb2/%d-%d_%d-%d_%d-%d_%d-%d/Sort_OniaTree_pPb2_PADoubleMuon_%d.root", (int)multMin, (int)multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, imass*(120/nmbin)+ibin);
				...
				if(ptMin <= trg_pt && trg_pt < ptMax && rapMin <= trg_y && trg_y < rapMax)
				{
					for(Int_t itrk = 0; itrk < Nass; itrk++)
					{
						...
						if(TrkptMin <= ass_pt && ass_pt < TrkptMax)
						{
							...
							h1->Fill(deta, dphi, 1/(double)TrueNtrg);
							if(fabs(deta) > 2.0) h2->Fill(deta, dphi, 1/(double)TrueNtrg);
							if(fabs(deta) > 1.0) h3->Fill(deta, dphi, 1/(double)TrueNtrg);
						}
					}
				}
			}
...
//store{{{
	TFile* fout = new TFile(Form("%d-%d_%d-%d_%d-%d_%d-%d_%d/deta-dphi_pPb_distribution_sig_%d.root", (int)multMin, (int)multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, nmbin, imass), "RECREATE");
	fout->cd();
	h1_1->Write();
	h1_2->Write();
	h2_1->Write();
